    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="d-flex flex-column flex-md-row align-items-center gap-4">
          ...
          <div class="flex-grow-1 text-center text-md-start">
            ...
            <p class="text-muted mb-2">{{number_format($patient->age,0)}} anos • {{$patient->objective}} • {{number_format($patient->weight,2)}} kg</p>
            <div class="d-flex justify-content-center justify-content-md-start gap-4 text-sm">
              <div><strong>Última Dieta:</strong> {{ $lastDiet->uploaded_at->format('d/m/Y') }}</div>
              {{-- <div><strong>Calorias Médias:</strong> 1.850 kcal</div> --}}
            </div>
          </div>

          <div>
            <a href="{{route("diets.create",["patient"=>$patient->id])}}" class="btn btn-success rounded-pill px-4 py-2">
        ...
        <div class="card diet-card h-100 border-0 shadow-sm">
          <div class="card-body">
            ...
            <div class="d-flex gap-2">
              <a href="{{route("diets.show",["patient"=>$patient,"diet"=>$diet])}}" class="btn btn-outline-success flex-grow-1 btn-sm">
